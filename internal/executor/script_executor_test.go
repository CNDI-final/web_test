package executor

import (
	"context"
	"testing"
	"time"

	"web_test/pkg/models"

	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/mock"
)

// MockTaskQueue 模擬任務佇列
type MockTaskQueue struct {
	mock.Mock
}

func (m *MockTaskQueue) PushTask(ctx context.Context, task *models.Task) error {
	args := m.Called(ctx, task)
	return args.Error(0)
}

func (m *MockTaskQueue) PopTask(ctx context.Context) (*models.Task, error) {
	args := m.Called(ctx)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).(*models.Task), args.Error(1)
}

func (m *MockTaskQueue) GetTasks(ctx context.Context) ([]*models.Task, error) {
	args := m.Called(ctx)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).([]*models.Task), args.Error(1)
}

func (m *MockTaskQueue) RemoveTask(ctx context.Context, taskID string) error {
	args := m.Called(ctx, taskID)
	return args.Error(0)
}

// MockResultStore 模擬結果儲存（實現 database.ResultStore 介面）
type MockResultStore struct {
	mock.Mock
	SavedResults map[string]*models.TaskResult
}

func NewMockResultStore() *MockResultStore {
	return &MockResultStore{
		SavedResults: make(map[string]*models.TaskResult),
	}
}

func (m *MockResultStore) SaveResult(ctx context.Context, result *models.TaskResult) error {
	args := m.Called(ctx, result)
	if args.Error(0) == nil {
		m.SavedResults[result.TaskID] = result
	}
	return args.Error(0)
}

func (m *MockResultStore) GetResult(ctx context.Context, taskID string) (*models.TaskResult, error) {
	args := m.Called(ctx, taskID)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).(*models.TaskResult), args.Error(1)
}

func (m *MockResultStore) DeleteResult(ctx context.Context, taskID string, status string) error {
	args := m.Called(ctx, taskID, status)
	return args.Error(0)
}

func (m *MockResultStore) GetRunningTasks(ctx context.Context) ([]*models.TaskResult, error) {
	args := m.Called(ctx)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).([]*models.TaskResult), args.Error(1)
}

func (m *MockResultStore) IncrementTaskID(ctx context.Context) (int, error) {
	args := m.Called(ctx)
	return args.Int(0), args.Error(1)
}

func (m *MockResultStore) SaveHistory(ctx context.Context, record *models.HistoryRecord) error {
	args := m.Called(ctx, record)
	return args.Error(0)
}

func (m *MockResultStore) GetHistory(ctx context.Context) ([]*models.HistoryRecord, error) {
	args := m.Called(ctx)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).([]*models.HistoryRecord), args.Error(1)
}

func (m *MockResultStore) SavePrCache(ctx context.Context, Prs []byte) error {
	args := m.Called(ctx, Prs)
	return args.Error(0)
}

func (m *MockResultStore) GetPrCache(ctx context.Context) ([]byte, error) {
	args := m.Called(ctx)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).([]byte), args.Error(1)
}

// TestProcessTaskSuccess 測試成功處理任務
func TestProcessTaskSuccess(t *testing.T) {
	// 準備
	mockQueue := new(MockTaskQueue)
	mockStore := NewMockResultStore()

	task := &models.Task{
		ID: "task-123",
		Params: []models.TaskParams{
			{NF: "amf", PRVersion: "70"},
			{NF: "smf", PRVersion: "80"},
		},
	}

	// 設置 mock 期望
	mockQueue.On("PopTask", mock.Anything).Return(task, nil)
	mockStore.On("SaveResult", mock.Anything, mock.MatchedBy(func(r *models.TaskResult) bool {
		return r.TaskID == task.ID && r.Status == "running"
	})).Return(nil)
	mockStore.On("SaveResult", mock.Anything, mock.MatchedBy(func(r *models.TaskResult) bool {
		return r.TaskID == task.ID && r.Status == "success"
	})).Return(nil)
	mockStore.On("DeleteResult", mock.Anything, task.ID, "running").Return(nil)

	executor := &TaskExecutor{
		queue: mockQueue,
		db:    mockStore,
	}

	// 執行
	ctx := context.Background()
	err := executor.processNextTask(ctx)

	// 驗證
	assert.NoError(t, err)
	assert.NotNil(t, mockStore.SavedResults[task.ID])
	assert.Equal(t, "success", mockStore.SavedResults[task.ID].Status)
	mockQueue.AssertExpectations(t)
	mockStore.AssertExpectations(t)
}

// TestProcessTaskWithFailedTests 測試包含失敗測試的任務
func TestProcessTaskWithFailedTests(t *testing.T) {
	mockQueue := new(MockTaskQueue)
	mockStore := NewMockResultStore()

	task := &models.Task{
		ID: "task-456",
		Params: []models.TaskParams{
			{NF: "amf", PRVersion: "70"},
		},
	}

	failedOutput := `
					TestAMF started
					Running test cases...
					FAIL: TestAMF_Case1
					Error details here
					TestAMF completed
					`

	mockQueue.On("PopTask", mock.Anything).Return(task, nil)
	mockStore.On("SaveResult", mock.Anything, mock.MatchedBy(func(r *models.TaskResult) bool {
		return r.TaskID == task.ID && r.Status == "running"
	})).Return(nil)
	mockStore.On("SaveResult", mock.Anything, mock.MatchedBy(func(r *models.TaskResult) bool {
		return r.TaskID == task.ID && r.Status == "failed"
	})).Return(nil)
	mockStore.On("DeleteResult", mock.Anything, task.ID, "running").Return(nil)

	failedLogs := parseFailedTestLogs(failedOutput)
	assert.NotEmpty(t, failedLogs, "應該解析到失敗的測試")
}

// TestExecuteTaskSuccess 測試 executeTask 成功情況
func TestExecuteTaskSuccess(t *testing.T) {
	mockStore := NewMockResultStore()
	mockQueue := new(MockTaskQueue)

	executor := &TaskExecutor{
		queue: mockQueue,
		db:    mockStore,
	}

	task := &models.Task{
		ID: "task-789",
		Params: []models.TaskParams{
			{NF: "upf", PRVersion: "65"},
		},
	}

	results := executor.executeTask(context.Background(), task)
	assert.NotEmpty(t, results)
	assert.Equal(t, task.ID, results[0].TaskID)
}

// TestParseFailedTestLogs 測試解析失敗測試日誌
func TestParseFailedTestLogs(t *testing.T) {
	tests := []struct {
		name     string
		output   string
		expected map[string]string
	}{
		{
			name: "單個失敗測試",
			output: `TestAMF started
Running test case 1...
FAIL: TestAMF
Error: connection timeout
TestAMF completed`,
			expected: map[string]string{
				"TestAMF": "包含失敗標記",
			},
		},
		{
			name: "無失敗測試",
			output: `TestAMF started
Running test case 1...
PASS: TestAMF
TestAMF completed`,
			expected: map[string]string{},
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := parseFailedTestLogs(tt.output)
			if len(tt.expected) == 0 {
				assert.Empty(t, result)
			} else {
				assert.NotEmpty(t, result)
			}
		})
	}
}

// TestProcessTaskContextCancellation 測試 context 取消
func TestProcessTaskContextCancellation(t *testing.T) {
	mockQueue := new(MockTaskQueue)
	mockStore := NewMockResultStore()

	ctx, cancel := context.WithCancel(context.Background())
	cancel()

	mockQueue.On("PopTask", ctx).Return(nil, context.Canceled)

	executor := &TaskExecutor{
		queue: mockQueue,
		db:    mockStore,
	}

	err := executor.processNextTask(ctx)
	assert.Equal(t, context.Canceled, err)
}

// TestIntegrationSuccessfulTaskProcessing 集成測試
func TestIntegrationSuccessfulTaskProcessing(t *testing.T) {
	mockQueue := new(MockTaskQueue)
	mockStore := NewMockResultStore()

	task := &models.Task{
		ID: "integration-task-001",
		Params: []models.TaskParams{
			{NF: "amf", PRVersion: "70"},
			{NF: "smf", PRVersion: "80"},
		},
	}

	mockQueue.On("PopTask", mock.Anything).Return(task, nil).Once()
	mockStore.On("SaveResult", mock.Anything, mock.MatchedBy(func(r *models.TaskResult) bool {
		return r.Status == "running"
	})).Return(nil).Once()
	mockStore.On("SaveResult", mock.Anything, mock.MatchedBy(func(r *models.TaskResult) bool {
		return r.Status == "success"
	})).Return(nil).Once()
	mockStore.On("DeleteResult", mock.Anything, task.ID, "running").Return(nil).Once()

	executor := &TaskExecutor{
		queue: mockQueue,
		db:    mockStore,
	}
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	err := executor.processNextTask(ctx)

	assert.NoError(t, err)
	assert.Equal(t, 1, len(mockStore.SavedResults))
	assert.Equal(t, task.ID, mockStore.SavedResults[task.ID].TaskID)

	mockQueue.AssertExpectations(t)
	mockStore.AssertExpectations(t)
}
